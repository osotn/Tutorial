...
0x08, 0x00, 0x45, 0x00,
0x00, 0x2f, 0x00, 0x00, 0x40, 0x00, 0x40, 0x11, 0x2f, 0xb4,
...
{
        {
            // UDP Check Sum:
            UINT_32   sum = 0;
            UINT_16  *ip_src = (UINT_16*)(pkt_PTR + 26);
            UINT_16  *ip_dst = (UINT_16*)(pkt_PTR + 30);
            UINT_16  *length = (UINT_16*)(pkt_PTR + 38);

            UINT_16  *buf = (UINT_16*)(pkt_PTR + 34);
            UINT_16   len = *length;
                      len = RSG_swap16_MAC(len);
            ...
            sum += *ip_dst;
            sum += *length;
            sum += RSG_swap16_MAC(0x11); /* UDP protocol */
            ...
            *(UINT_16 *)(pkt_PTR + data_start - 2) = check_sum; // no swap
                ...
                if (s_old_check_sum != check_sum) {
                    ...
                    for (i = 0; i < RRPC_pkt_size_bcast_CNS; i++) {
                       ...
                       if (i >= 40 && i <= 41)
                            ...
                            printf(TTY_NORMAL); printf(" %3d = 0x%02x ", i, *(UINT_8 *)(pkt_PTR+i));
                    }
                    printf("\n");
