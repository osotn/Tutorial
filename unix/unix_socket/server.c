...
int main()
{
	...
	if( bind(serv_sock, (struct sockaddr*)&local, len) != 0)
	{
		printf("Error on binding socket \n");
		return 1;
	}

	if( listen(serv_sock, nIncomingConnections) != 0 )
	{
	...
	while (bWaiting)
	{
		...
		if( (s2 = accept(serv_sock, (struct sockaddr*)&remote, &sock_len)) == -1 )
		{
